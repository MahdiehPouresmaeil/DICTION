...
from watermark.hufunet import extract
...
def train_student_hufu(student, teacher, loader, temperature=2.0, lr=1e-3, epochs=3, supp=None, device="cuda", extract=None, layer_name=None, hufu_model=None,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(loop):
            ...
            # mse_before, mse_after, mse_non_wm=extract(deepcopy(student),deepcopy( hufu_model), selected_indexes, train_loader_hufu,
            #         config)
            ...
            loop.set_postfix(loss=train_loss / (batch_idx + 1), acc=100. * correct / total,
                             ...
                             # mse_before=f"{mse_before:4f}",
                             # mse_after=f"{mse_after:4f}"
                             # , error_wat=f"{error_wat:.4f}"
                             )
        mse_before, mse_after, mse_non_wm = extract(deepcopy(student), deepcopy(hufu_model), selected_indexes,
                                                    train_loader_hufu,
                                                    config)
        print(f"student model: Epoch [{epoch}/{epochs}]    mse_hufu: {mse_before}, mse_student: {mse_after}")
