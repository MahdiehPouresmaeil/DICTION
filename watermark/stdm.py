...
def theta(x,alpha, beta):
    numerator = torch.exp(alpha * torch.sin(torch.tensor(beta) * x))
    denominator = 1 + torch.exp(alpha * torch.sin(torch.tensor(beta)* x))
...
def embed(init_model, test_loader, train_loader, config) -> object:
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch_idx, (inputs, targets) in enumerate(loop):
            ...
            matrix_g = theta(selected_weights @ matrix_a, config["alpha"], config["beta"])
            ...
            _, ber_ = _extract(model, matrix_a, watermark, config["layer_name"], config["alpha"], config["beta"])
...
def extract(model_watermarked, supp):
    return _extract(model_watermarked, supp["matrix_a"], supp["watermark"], supp["layer_name"], supp["alpha"], supp["beta"])


def _extract(model, matrix_a, watermark, layer_name, alpha_param, beta_param):
    ...
    g_ext = theta(w @ matrix_a,alpha_param, beta_param)
    wat_ext = (g_ext > 0.5) * 1.
